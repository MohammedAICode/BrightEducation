...
const Teachers: React.FC = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(`/user/all?page=${page}&limit=${limit}&role=TEACHER${searchParam}${statusParam}`);
      ...
      <div className="bg-white rounded-xl shadow-xs border border-gray-100 overflow-hidden">
        ...
        {!loading && !error && (
          <div className="overflow-x-auto">
            {users.length === 0 ? (
              <div className="text-center py-12 text-gray-500 font-medium">No teachers found</div>
            ) : (
              <>
